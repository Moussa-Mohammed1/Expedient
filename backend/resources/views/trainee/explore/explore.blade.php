    ...
    <section class="max-w-7xl mx-auto px-6 lg:px-10 mb-16">
        <form action="{{ route('explore') }}" method="GET" class="relative max-w-4xl mx-auto group">
            ...
            <div class="absolute inset-y-0 right-2 flex items-center">
                <button type="submit" class="bg-[#1c1c1c] cursor-pointer hover:bg-yellow-500 hover:text-black text-white text-xs font-bold py-2 px-4 rounded-2xl border border-zinc-700  duration-300">
                    Search
                </button>
            </div>
        </form>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @forelse($coaches as $coachUser)
                        <a href="{{ route('coaches.show', $coachUser->coach->id) }}"
                            ...
                            <div class="relative">
                                <img
                                    src="{{ $coachUser->avatar ? asset('/storage/users/profiles/' . $coachUser->avatar) : asset('assets/images/profile.jpeg')}}"
                            ...
                            <div class="min-w-0 flex-col">
                                ...
                                <p class="text-zinc-500 text-xs mt-0.5 truncate">{{ $coachUser->localisation ?: 'Location not set' }}</p>
                                <p class="text-zinc-500 text-[10px] mt-1 truncate">{{ optional($coachUser->coach?->salles->first())->name ?: 'No salle assigned yet' }}</p>
                            </div>
                        </a>
                    @empty
